---
origem: brief institucional (AVA — IA PARA DESENVOLVEDORES)
status: referência
codigo: Projeto Avaliativo – M2.2
peso: 60% da nota do módulo
liberacao: 2026-08-21 22h
entrega: 2026-08-31 15h
---
...
1. [Contextualização](#1-contextualizacao)
2. [Desafio](#2-desafio)
3. [Resultados Esperados (Entrega)](#3-resultados-esperados-entrega)
4. [Requisitos da Aplicação](#4-requisitos-da-aplicacao)
   - 4.1. [Domínio, escopo e cenários](#41-domínio-escopo-e-cenários)
   - 4.2. [Arquitetura agêntica e LangGraph](#42-arquitetura-agêntica-e-langgraph)
   - 4.3. [Tools, MCP e integrações](#43-tools-mcp-e-integrações)
   - 4.4. [Memória, contexto e RAG](#44-memória-contexto-e-rag)
   - 4.5. [Segurança, governança e limites de autonomia](#45-segurança-governança-e-limites-de-autonomia)
   - 4.6. [Observabilidade e resiliência](#46-observabilidade-e-resiliência)
   - 4.7. [IA para QA e testes inteligentes](#47-ia-para-qa-e-testes-inteligentes)
   - 4.8. [DevOps inteligente e detecção de falhas](#48-devops-inteligente-e-detecção-de-falhas)
   - 4.9. [Low-Code para QA, SRE e agentes](#49-low-code-para-qa-sre-e-agentes)
   - 4.10. [Prompts, modelos e refinamento](#410-prompts-modelos-e-refinamento)
5. [Roteiro da Aplicação](#5-roteiro-da-aplicacao)
   - 5.1. [Formato do sistema](#51-formato-do-sistema)
   - 5.2. [Documentação no README.md](#52-documentação-no-readmemd)
   - 5.3. [Uso do quadro do GitHub](#53-uso-do-quadro-do-github)
   - 5.4. [Uso do repositório no GitHub](#54-uso-do-repositório-no-github)
   - 5.5. [Gravação de vídeo](#55-gravação-de-vídeo)
...
- Aplicar IA em revisão de código, geração ou refinamento de testes e priorização orientada por risco;
...
| Item | Valor |
|---|---|
| Peso do projeto | Avaliação M2.2 — 60% da nota do módulo |
...
- **Extensão opcional:** o aluno poderá utilizar ChatOps ou outro mecanismo de notificação para comunicar resultados, alertas ou diagnósticos produzidos pela aplicação, utilizando, por exemplo, Discord, Slack, Microsoft Teams, e-mail, GitHub Issue ou webhook.
...
| 2:00 a 4:00 | Dois cenários de uso (fluxo principal + cenário de risco, falha, exceção ou comportamento anômalo) |
...
|---|---|---|---|
| 1 | O estudante entregou o vídeo de demonstração no YouTube como não listado, com duração recomendada de até 10 minutos e limite máximo de 12 minutos, cobrindo os pontos descritos no item 5.5? | O vídeo não foi entregue, está inacessível, ultrapassa 12 minutos ou não demonstra o funcionamento e as evidências técnicas solicitadas. | O vídeo está acessível, respeita o limite máximo de 12 minutos e cobre de forma clara os pontos previstos no item 5.5. |

### 6.2. Uso adequado do quadro do GitHub

| Nº | Critério de Avaliação | 0 | 0,25 | 0,50 |
|---|---|---|---|---|
| 2 | O estudante organizou o escopo do projeto em cards no quadro do GitHub? | O quadro não existe, está inacessível ou não possui cards relacionados ao projeto. | Os cards existem, mas representam apenas parte do trabalho ou possuem descrições pouco claras sobre a atividade e o resultado esperado. | Os cards representam de forma coerente as principais atividades do projeto e possuem descrições suficientes para compreender o que foi planejado e realizado. |
| 3 | O estudante manteve o quadro atualizado durante o desenvolvimento? | Os cards não foram movimentados ou foram organizados apenas ao final do projeto. | A movimentação ocorreu de forma parcial ou irregular, sem representar claramente a evolução do trabalho. | O andamento dos cards foi atualizado de forma coerente, permitindo acompanhar a evolução real do projeto durante o desenvolvimento. |

### 6.3. Uso adequado do GitHub e README.md

| Nº | Critério de Avaliação | 0 | 0,25 | 0,75 |
|---|---|---|---|---|
| 4 | O estudante utilizou adequadamente branches, commits e o fluxo de versionamento do projeto? | O repositório não existe, está inacessível ou concentra o desenvolvimento diretamente na main, sem histórico que permita compreender a evolução do projeto. | O repositório utiliza branches, mas apresenta fluxo inconsistente, histórico concentrado em poucas alterações ou commits com mensagens genéricas e pouco representativas. | O desenvolvimento utiliza de forma coerente develop, feature branches e main, com commits incrementais e mensagens semânticas, claras e relacionadas à evolução real do projeto. |
| 5 | O README.md e a documentação permitem compreender, executar e avaliar a solução? | O README.md está ausente ou não fornece informações suficientes para compreender e executar o projeto. | O README.md existe, mas possui instruções incompletas, informações importantes ausentes ou documentação insuficiente para compreender plenamente a solução. | O README.md apresenta de forma clara a solução, sua arquitetura, instruções de configuração e execução, principais decisões técnicas, cenários e evidências necessárias para compreender, reproduzir e avaliar o projeto. |

### 6.4. Aplicação, arquitetura agêntica e integrações

| Nº | Critério de Avaliação | 0 | 0,25 | 0,75 |
|---|---|---|---|---|
| 6 | A aplicação está funcional e demonstra adequadamente o problema e os cenários definidos? | A aplicação não executa, depende exclusivamente de respostas fixas ou não permite compreender o problema que pretende resolver. | A aplicação executa parcialmente ou demonstra apenas um dos cenários esperados, apresentando limitações relevantes no fluxo. | A aplicação executa de ponta a ponta, possui problema e domínio claramente definidos, demonstra os dois cenários esperados e produz uma saída estruturada e adequada ao domínio. |
| 7 | O fluxo foi modelado adequadamente com LangGraph? | Não há LangGraph funcional ou o fluxo não possui state, nodes e edges compreensíveis. | O grafo existe, mas possui estado confuso, nodes excessivamente acoplados ou controle de fluxo incompleto. | O grafo possui state tipado, nodes claros, execução sequencial, ramificação condicional, paralelização simples, condição de parada e separação coerente entre decisões do modelo e regras determinísticas. |
| 8 | A solução utiliza adequadamente uma tool integrada? | Não há tool funcional ou integração demonstrável. | A tool existe, mas apresenta integração, validação ou tratamento de falhas incompleto. | A solução possui uma tool funcional integrada por MCP, API, serviço, backend ou webhook, com validação e tratamento de falhas. |
| 9 | A solução utiliza uma estratégia de memória ou recuperação contextual adequada ao domínio? | Não há estratégia de memória ou recuperação contextual, ou o contexto é inserido de forma fixa e descontrolada. | Existe uma estratégia de contexto, memória ou recuperação, mas sua utilização é limitada, pouco documentada ou não demonstra claramente o uso de informações relevantes pela aplicação. | A solução utiliza de forma coerente uma estratégia de memória ou recuperação contextual adequada ao domínio, como state, checkpointer, armazenamento persistente ou RAG, recuperando e utilizando informações relevantes conforme a necessidade da aplicação. |

### 6.5. Segurança, observabilidade e resiliência

| Nº | Critério de Avaliação | 0 | 0,25 | 0,75 |
|---|---|---|---|---|
| 10 | A solução aplica controles adequados de segurança e limites de autonomia? | Não há proteção adequada de segredos, validação das ações ou tratamento de entradas não confiáveis. | Existem controles de segurança, mas estão incompletos ou o cenário adversarial não demonstra claramente o comportamento seguro da aplicação. | A solução protege credenciais, valida ações e entradas e demonstra um cenário adversarial no qual ações não autorizadas são bloqueadas. Aprovação humana é utilizada quando aplicável ao domínio. |
| 11 | A execução possui pelo menos dois sinais de observabilidade correlacionados e tratamento adequado de falhas? | Não há sinais suficientes para investigar a execução e as falhas não são tratadas adequadamente. | Há apenas um sinal, os sinais não possuem correlação suficiente ou o tratamento de falhas está incompleto. | A solução produz e correlaciona pelo menos dois sinais de observabilidade, incluindo logs estruturados, e permite identificar decisões, erros e latência. Também trata falhas com timeout, retry ou fallback, quando necessário. |

### 6.6. QA, DevOps Inteligente e Low-Code

| Nº | Critério de Avaliação | 0 | 0,25 | 0,50 |
|---|---|---|---|---|
| 12 | O estudante aplicou IA em code review e testes relevantes? | Não há evidência relevante de uso de IA em revisão de código ou testes. | Há code review ou testes gerados, mas com cobertura superficial, sem validação crítica ou priorização por risco. | A IA é utilizada em code review de uma alteração real e na geração ou refinamento de testes relevantes, incluindo pelo menos um dos seguintes tipos: integração, aceitação ou E2E, com priorização por risco ou impacto. |
| 13 | A solução contempla DevOps inteligente, detecção de anomalias e estimativa de tendência ou risco de falha? | Não há pipeline funcional nem análise de logs, anomalias ou estimativa de risco de falha. | Há pipeline e uma análise parcial, mas faltam etapas, sinais, evidências ou interpretação estruturada. | O pipeline executa lint, testes e build ou validação equivalente; a IA explica logs de pelo menos duas etapas, detecta e explica uma anomalia e produz uma estimativa simples de tendência ou risco. O deploy é considerado apenas quando aplicável. |
| 14 | O estudante integrou uma automação low-code ou no-code ao projeto? | Não há fluxo low-code/no-code demonstrável. | Existe um fluxo visual isolado, incompleto ou sem integração real com a aplicação. | Há automação low-code ou no-code integrada à solução principal, com trigger, integração com a aplicação ou serviço, saída observável e instruções de reprodução. |

### 6.7. Análise crítica e evidências

| Nº | Critério de Avaliação | 0 | 0,25 | 0,50 |
|---|---|---|---|---|
| 15 | O estudante documentou um refinamento relevante da solução e apresentou evidências do desenvolvimento? | Não há análise crítica ou evidências de refinamento. | Há menção a um problema ou alteração, mas sem mostrar claramente a decisão e o resultado. | O estudante apresenta o problema ou comportamento observado, a alteração realizada, a justificativa, o resultado obtido e evidências relacionadas ao desenvolvimento. |
