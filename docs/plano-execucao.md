...
| 31/08 | **Feature extra (Parte 9)**: recomendação por carrinho/produto semente (`seed_products: list[int]`, opcional, máx. 10) — coocorrência market-basket com base no carrinho; 11 testes novos em `tests/test_carrinho.py`; 57 passed · 0 regressões; Ruff limpo; teste visual via Swagger (`/docs`); 2 bugs colaterais corrigidos (erros entre nodes + vazamento entre chamadas) |
